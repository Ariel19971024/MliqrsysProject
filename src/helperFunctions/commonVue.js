...
// 	//formatDate is from another plugin
// // }
...
//  */
// /**
//  * function setErrorMessage(msg) { var error = "<div class=\"info\">"; error =
//  * error + "<img style=\"vertical-align: middle\" src=\"" + contextPath +
//  * "/images/common/information.gif\">"; error = error + "<span
//  * class=\"actionMessage\">" + msg + "</span>"; error = error + "<br>"; error =
//  * error + "</div>"; return error; } function messageErrorBox(msg, title) { new
//  * $.msgbox({ type : 'alert', title : title, content : msg, }).show();
//  * $("#jMsgbox_content").css("height", "100px"); } function messageBox(msg,
//  * title) { new $.msgbox({ type : 'alert', title : title, content : msg
//  * }).show(); $("#jMsgbox_content").css("height", "60px"); }
//  */
...
//  * @param tempStr
//  */
// function toDigitalEnglishUpper1(obj) {
...
// /**
//  * (僅針對chungkuo專案使用)(僅針對共用視窗模組使用)清除共用視窗input旁的span做清除
//  * 
//  * @Param idArray
//  *            input有使用共用視窗的id陣列
//  */
...
// /**
//  * (僅針對chungkuo專案使用)錯誤訊息處理,例:輸入「{0}foobar{1}」和「{"hello","world"}」,回傳為hellofoobarworld
//  */
...
// /**
//  * (僅針對chungkuo專案)
//  * 依照定義的json格式和內容,產生相對應的button碼(注意!每一個物件會有td在前後,例:＜ｔｄ＞＜ａ＞...＜／ａ＞＜／ｔｄ＞)
...
// // 		//isEmpty is not defined or incorrect usage of str.isEmpty()
// // 	}
...
//  */
// /*
//  * function messageErrorBoxType2(msg, title, buttonJsonObj) { // new Dialog
//  * Object // 此機制有無法操作多個dialog的問題 var DialogObjectTemp = new $.msgbox({ type :
//  * 'alert', title : title, content : msg });
//  * 
//  * DialogObjectTemp.show(); $("#jMsgbox_content").css("height", "100px"); //
//  * 清空既有button DialogObjectTemp.mainWrap.find("input[type=button]")[0].outerHTML =
//  * ""; // 建立新的Button var a = buttonJsonObj.BUTTON; var html = "";
//  * 
//  * for (var i = 0; i < a.length; i++) { var buttonValTemp = a[i].buttonVal; html += "<input
//  * type='button' name='' value='" + buttonValTemp + "'>" + "<div
//  * style='display:inline;'> </div>"; } // html = "<div
//  * style='text-align:center;'>" + html + "</div>"; html = "<center
//  * style='padding: 8px;'>" + html + "</center>";
//  * DialogObjectTemp.mainWrap.append(html); // 重新註冊click event var buttonArray =
//  * DialogObjectTemp.mainWrap.find("input[type=button]");
//  * 
...
//  */
// /*
//  * function settingErrMsgButton(jsonObj) { if (jsonObj != null &&
//  * window.errorMsg && !checkIsNullSpace(errorMsg) && errorMsg.length > 0) { if
//  * ($("# _content").find("div").html().length > 0) { // 清空原來的button
//  * $("#jMsgbox_content").parent().find("input[type=button]")[0].outerHTML = ""; //
//  * 建立新的Button var a = jsonObj.BUTTON; var html = "";
//  * 
//  * for (var i = 0; i < a.length; i++) { var buttonValTemp = a[i].buttonVal; html += "<input
//  * type='button' name='' value='" + buttonValTemp + "'>";
//  * 
//  * if (i != a.length - 1) { html += "<div
//  * style='display:inline;'>&nbsp;&nbsp;&nbsp;</div>"; } } // html = "<div
//  * style='text-align:center;'>" + html + // "</div>";//排版有問題,改用以下方式 html = "<center
//  * style=\"padding: 8px;\">" + html + "</center>";
//  * $("#jMsgbox_content").parent()[0].innerHTML =
//  * $("#jMsgbox_content").parent()[0].innerHTML + html; // 重新註冊click event var
//  * buttonArray = $("#jMsgbox_content").parent().find("input[type=button]");
//  * 
//  * for (var i = 0; i < buttonArray.length; i++) {
...
// // 	// contextPath + "/jsps/relay.jsp"為僅針對chungkuo專案使用
...
//  * @param tempStr
//  */
// function toDigitalEnglishUpper2(obj) {
// 	var val = obj.value.toUpperCase();
 ...
 * @returns {Boolean}
 */
export function checkIsNullSpace(val) {
...
// 	var fieldobject = getObject(fieldName);
// 	var val = fieldobject.value;
// 	var data = val.match(/^[09]{2}[0-9]{2}-[0-9]{6}$/);
// 	if (!data || !val)
// 		return showErrorCommon(fieldobject, msg);
// 	if (val.indexOf(';') != -1)
// 		return showErrorCommon(fieldobject, msg);
...
// 	var data = val.match(/^[0-9]{2}-[0-9]{7,8}(?:(?:#)(\d+))?$/);
...
// 	var s = parseInt(id.substr(0, 1)) + parseInt(id.substr(1, 1)) * 9 + parseInt(id.substr(2, 1)) * 8
...
// 	var vhclInsPdtDataJson = JSON.parse(vhclInsPdtData);// 設為全域變數,待後續可以使用
...
// 		tempJsonStr = JSON.stringify(tempJson);
// 	}
...
//  * @returns
//  */
export function domain4Springboot(isprotocol) {
	var mode = "eclipse"; //dev、sit、uat、prod、eclipse
	var domains = document.URL.split("/");

	if (isprotocol == true) {
		if (mode != "sit" && mode != "uat" && mode != "prod") {
			return genUrlByMode(isprotocol, mode);
		}

		return domains[0] + "//" + domains[2]
	}
	if (isprotocol == false) {
		if (mode != "sit" && mode != "uat" && mode != "prod") {
			return genUrlByMode(isprotocol, mode);
...
/**
 */
export function genUrlByMode(isprotocol, mode) {
	var urldev = "ckra-ck.apps.ocp45.asgard.com.tw";
	var urleclpse = "localhost:10202";

	if (mode === "dev") {
		return isprotocol == true ? "http://" + urldev : urldev;
	} else if (mode === 'eclipse') {
		return isprotocol == true ? "http://" + urleclpse : urleclpse;
	}
}
...
//  * @returns
//  */
function fetchApiResFromCache(apiUrl, param, method) {
	var paramstr = JSON.stringify(param);
	var k = sha1(method + paramstr + apiUrl);
...
// /**
//  * (僅針對chungkuo專案)針對共用視窗的input,如果該input是手動輸入,則會發動Ajax查詢資料_Type2
//  * 
//  * @param thisObj
//  * @param actionUrl
...
// /**
//  * (僅針對chungkuo專案)針對共用視窗的input,如果該input是手動輸入,則會發動Ajax查詢資料
 ...
 * @returns
 */
export function ajaxPostTokenReady(url, param, isCache) {
 ...
 * @returns
 */
export function ajaxGetTokenReady(url, param, isCache) {
	var token = fetchToken();
...
/**
 * ajaxPostTokenReady(使用 RequestBody 時需使用此方法)
 * 
 * @param url
 * @param param
 * @param isCache
...
function ajaxPostWithCache(url, paramJson, token) {
	var cacheData = fetchApiResFromCache(url, paramJson, "POST");
...
/**
 * ajaxPost(使用 RequestBody 時需使用此方法)
 * 
 * @param url
 * @param paramJson
 * @param token
...
/**
 * ajax-get(含Cahce機制，$.ajax本身有cache機制，但因為跨頁即失效，故還是需要全域變數概念的cache)
 * 
 * @param url
 * @param paramJson
 * @param token
...
export function numCalculation(num1, num2, operation) {
	let num1d = countDecimals(num1);
	let num2d = countDecimals(num2);
	let ans;
	if (operation == 'add') {
		ans = (num1 + num2).toFixed(Math.max(num1d, num2d))
	} else if (operation == 'subtract') {
		ans = (num1 - num2).toFixed(Math.max(num1d, num2d))
	} else if (operation == 'multiply') {
		ans = (num1 * num2).toFixed(num1d + num2d)
	}
...
export function moneyFormatter(num) {
	...
	return formatter.format(num).slice(1)
}
...
function modifyKeys(obj) {
	...
	for (let i = 0; i < keys.length; i++) {
		newObj[modifyKey(keys[i])] = obj[keys[i]];
	}
	return newObj;
}
...
function modifyKey(key) {
	//key => str
	let newKey = key.replace(/(?:^|\.?)([A-Z])/g, function (x, y) {
		return "_" + y.toLowerCase()
	}).replace(/^_/, "");
	return newKey
}

export function populateTable(newTableData, tabledata) {
	let newTableDataIds = Object.keys(newTableData);
	for (let i = 0; i < tabledata.length; i++){
		if(newTableDataIds.includes(tabledata[i].keyId)){			
			tabledata[i].value = newTableData[tabledata[i].keyId];
		}
	}
	return tabledata;
}