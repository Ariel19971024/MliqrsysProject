<template>
  ...
  <label>
    <input
      ...
      @change="handleDatePickerTWChange($event)"
      autocomplete="off"
      placeholder="查詢日期"
    />
    <!-- <input
...
export default {
  ...
  computed: {
    dateDisplayed: {
      get() {
        ...
        if (this.dateSaved) {
          let year = this.dateSaved.getFullYear() - 1911;
          let month = this.dateSaved.getMonth() + 1;
          let date = this.dateSaved.getDate();
          if (month < 10 && month > 0) {
            month = "0" + month;
          }
          if (date < 10 && date > 0) {
  ...
  mounted() {
    ...
    console.log(vm.dateSaved);
    let dateNative = vm.dateSaved ? new Date(vm.dateSaved) : new Date();
